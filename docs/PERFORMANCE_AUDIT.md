...
Das Skript ist manuell. Es hängt nicht an `unit-tests` und an keinem Workflow. Unter Linux ist der geführte Peak eine Stichprobe der VmRSS-Summe (Worker 50 ms, importtime 20 ms); VmHWM läuft nicht gegen die Grenze. Unter Windows ist die Kennzahl `PeakJobMemoryUsed` (echter Höchststand, dieselbe wie das Job-Object-Gate); abgetastet wird nur der Zeitpunkt der Abbruchprüfung.
