...
| jobs-filter-age-days | JobsPage (widget only) | (age_days spinbox) | Created but **not wired into filter form** | HIDE_FROM_NORMAL_UI | Einstellungen>Diagnose / never surface | Dead control — do **not** show in V2. Explicit REMOVE needs product approval (not deleted in this refactor). |
...
| id | Decision | Status |
| --- | --- | --- |
| jobs-filter-age-days | Do not surface in V2 UI. Keep code until explicit REMOVE approval. | RESOLVED (HIDE) |
| Postfach as main nav | New V2 surface absorbing ambiguous-mail + reply draft + Gmail connect from Lifecycle/Privacy | PLANNED |
| Lebenszyklus nav | Remove from primary nav; redistribute functions to Bewerbungen/Postfach/Interview | PLANNED |
| Protokolle nav | Move under Einstellungen > Erweitert / Diagnose | PLANNED |

**Blocking UNKNOWN count: 0** — Phase 2 may proceed.
