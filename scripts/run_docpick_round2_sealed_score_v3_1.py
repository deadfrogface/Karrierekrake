#!/usr/bin/env python3
"""Score sealed Round2 Docpick predictions with COMPLETE_GT_ONLY_V3_1_DATE_NORM.
...
OUT = ROOT / "tests" / "docpick_qwen35" / "regression_known_cvs_round2"
...
def _slice_metrics(rows: list) -> dict:
    from holdout_scorer_v3_complete_gt import aggregate_v3

    # wrap as single fake doc aggregate from rows
    docs = [{"document": "slice", "scored_rows": rows, "nicht_bewertbar": []}]
    return aggregate_v3(docs)
...
def main() -> int:
    ...
    # map pdf path from seal predictions
    manifest = json.loads(
        (
            ROOT
            / "tests"
    ...
    # Perfect core: all scored fields correct on a document
    perfect = 0
    ...
    def lang_slice(lang: str) -> dict:
        ...
        a_wrap = aggregate_v3(
            [{"document": d["document"], "scored_rows": d["scored_rows"], "nicht_bewertbar": d["nicht_bewertbar"]} for d in docs]
        )
    ...
    # Follow-up = second document in extract order
    ...
    out = {
        ...
        "disclaimer": (
            "Round2 re-extract after name-from-email + compact schema + docling cache. "
        ...
        "coverage": {
            ...
            "n_nicht_bewertbar_fields": sum(len(d.get("nicht_bewertbar") or []) for d in doc_results),
    ...
    # Mark seal scoring completed (append-only flag file; do not rewrite seal hashes)
    scored_flag = OUT / "PHASE_A_SCORED_FLAG.json"
    scored_flag.write_text(
        json.dumps({"gt_loaded_after_seal": True, "scored": True, "metric": METRIC_NAME}, indent=2),
        encoding="utf-8",
    )
