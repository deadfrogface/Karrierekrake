#!/usr/bin/env python3
"""Round4 sealed regression extract after Phase-3 prompt/norm fixes.

REGRESSION on known CVs — NOT an independent blind test / NOT a 99% claim.
Writes to tests/docpick_qwen35/regression_known_cvs_round4/.
"""
...
OUT = ROOT / "tests/docpick_qwen35/regression_known_cvs_round4"
...
def main() -> int:
    ...
    freeze = {
        "seal_type": "PARSER_CONFIG_SCORER_FREEZE_ROUND4",
        ...
        "changes": [
            "llm_preflight_before_docling",
            "stage_progress_and_cancel",
            "address_enrich_from_header_patterns",
            "street_house_split",
            "software_level_strip",
            "employment_pipe_and_table_merge",
            "month_year_norm",
        ],
        "llm": {
    ...
    seal = {
        "seal_type": "PHASE_A_PREDICTION_SEAL_ROUND4",
        "test_type": "REGRESSION_KNOWN_CVS_NOT_BLIND",
        "disclaimer": "Known CVs only. Not independent blind. Not a 99% claim.",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "gt_not_loaded": True,
