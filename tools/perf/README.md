...
Das Skript bricht einen Worker ab, wenn die Prozessgruppe 3_200_000_000 Bytes überschreitet, damit der Lauf unter dem bestehenden Gate bleibt. Das Intervall der Abbruchprüfung steht in `memory_accounting.sample_interval_ms` (Worker 50 ms, importtime 20 ms).

Gegen die Grenze läuft:

- Linux: die Summe der **VmRSS** der Prozessgruppe, Feld `sampled_group_rss_max_bytes` und `sampled_limit_metric_max_bytes`. `peak_is_sampled` ist wahr. **VmHWM** ist der High-Water-Mark des Einzelprozesses und läuft nicht gegen die Grenze.
- Windows: **PeakJobMemoryUsed** des Job Objects (`JOBOBJECT_EXTENDED_LIMIT_INFORMATION`), dieselbe Kennzahl wie das Peak-Gate aus #69 (Commit-High-Water der Prozessgruppe). Nicht Working Set und nicht `PeakProcessMemoryUsed`. `peak_is_sampled` ist falsch, weil der Wert der echte Höchststand ist. Abgetastet wird nur der Zeitpunkt der Abbruchprüfung. Das Feld heißt nur `sampled_limit_metric_max_bytes`.

Das Job Object wird nach `Popen` zugewiesen. Ein Enkel, der in diesem Fenster entsteht, liegt noch nicht im Job. Das Gate aus #69 vermeidet das mit `CREATE_SUSPENDED` und `ResumeThread`; der Benchmark braucht zusätzlich die stdout-JSON und behält die Lücke.
